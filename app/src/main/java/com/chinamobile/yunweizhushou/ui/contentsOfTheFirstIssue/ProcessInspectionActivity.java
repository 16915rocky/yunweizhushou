package com.chinamobile.yunweizhushou.ui.contentsOfTheFirstIssue;
...
public class ProcessInspectionActivity extends BaseActivity {
	...
	private void initView() {
		listView = (ListView) findViewById(R.id.process_inspection_listview);
		typeLayout = (LinearLayout) findViewById(R.id.topTitle);
		tableHeadLayout = (LinearLayout) findViewById(R.id.head);
		topTitleScrollView = (HorizontalScrollView) findViewById(R.id.horizontalScrollView1);
		getTitleBar().setMiddleText(
				getIntent().getStringExtra("PROCNAME") == null ? "" : getIntent().getStringExtra("PROCNAME"));
	...
	private void initRequest() {
		...
		startTask(HttpRequestEnum.enum_takara_util, URL + param, map);
	}

	// 水平栏
	private void showTypeView(List<String> title) {
		for (int i = 0; i < title.size(); i++) {
			OperationTextView textView = new OperationTextView(ProcessInspectionActivity.this);
			...
			textView.setLayoutParams(
					new LinearLayout.LayoutParams(DisplayUtil.dip2px(ProcessInspectionActivity.this, width * 1.7f),
							DisplayUtil.dip2px(ProcessInspectionActivity.this, 50)));
	...
	private void setTableHead(int position) {
		...
		for (int i = 0; i < list.size(); i++) {
			TextView textView = new TextView(ProcessInspectionActivity.this);
			...
			// 如果大于4表头宽度为200，
			if (headSize > 4) {
				textView.setLayoutParams(new LayoutParams(DisplayUtil.dip2pxTyped(ProcessInspectionActivity.this, 80),
						DisplayUtil.dip2pxTyped(ProcessInspectionActivity.this, 40)));
			} else {
	...
	private class TextOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			Toast.makeText(ProcessInspectionActivity.this, textView.getText(), Toast.LENGTH_SHORT).show();
		}
	}
	...
	private class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			LinearLayout layout = new LinearLayout(ProcessInspectionActivity.this);
			...
			for (int i = p; i <= count; i++) {
				...
				if (headSize > 4) {
					textView.setLayoutParams(
							new LayoutParams(DisplayUtil.dip2pxTyped(ProcessInspectionActivity.this, 80),
									DisplayUtil.dip2pxTyped(ProcessInspectionActivity.this, 40)));
				} else {
					textView.setLayoutParams(new LayoutParams(ConstantValueUtil.WINDOW_WIDTH / headSize,
							DisplayUtil.dip2pxTyped(ProcessInspectionActivity.this, 40)));
