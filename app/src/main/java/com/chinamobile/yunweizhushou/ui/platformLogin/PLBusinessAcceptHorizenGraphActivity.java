...
public class PLBusinessAcceptHorizenGraphActivity extends BaseActivity implements OnClickListener {
	...
	private void initRequest() {
		...
		if (extraKey2 != null && !TextUtils.isEmpty(extraKey2)) {
	...
	protected void onTaskFinish(HttpRequestEnum e, ResponseBean responseBean) {
		...
		case enum_govern_analysis_graph_detail:
			...
			if (responseBean.isSuccess()) {
				...
				try {
					JSONObject jsonObject1 = new JSONObject(responseBean.getDATA());
					List<String> type = new ArrayList<>();
					if (jsonObject1.has("DOUBLEY")) {
						doubleY = jsonObject1.getString("DOUBLEY");
					}

					if (jsonObject1.has("COLUMNS")) {
						JSONArray array = jsonObject1.getJSONArray("COLUMNS");
					...
					if (jsonObject1.has("POINTS")) {
						...
						JSONArray arrays = jsonObject1.getJSONArray("POINTS");
					...
					if (jsonObject1.has("NAME")) {
						String str = jsonObject1.getString("NAME");
						title.setText(str);
					}
