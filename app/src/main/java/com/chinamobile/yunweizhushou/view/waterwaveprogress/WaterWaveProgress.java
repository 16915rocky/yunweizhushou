package com.chinamobile.yunweizhushou.view.waterwaveprogress;
...
public class WaterWaveProgress extends View {
	...
	float mProgress = 10, mMaxProgress = 100;
	...
	/** 波浪的速度 */
	private float mWaveSpeed = 0.030F; // 0.020F
	/** 水的透明度 */
	...
	protected void onDraw(Canvas canvas) {
		...
		float mTxtWidth = mTextPaint.measureText(mProgressStr, 0,
				mProgressStr.length());
		canvas.drawText(mProgressStr, mCenterPoint.x - mTxtWidth / 2,mCenterPoint.x*1.0f, mTextPaint);
		
		if (mShowNumerical) {
			
			DecimalFormat decimalFormat=new DecimalFormat("0.00");
			String num_str = decimalFormat.format((mProgress*1f) / mMaxProgress* 100f);
			num_str+="%";	
						
			float mTxtWidth_num = mTextPaint.measureText(num_str, 0,
					num_str.length());
			canvas.drawText(num_str, mCenterPoint.x - mTxtWidth_num / 2,mCenterPoint.x*1.6f, mTextPaint);
		}
	}
...
//	}
	...
	public void setProgress(float progress) {
		...
		mProgress = progress;
		if (progress<=50){
			setmWaterColor(getResources().getColor(R.color.guard_red));
		}else if (progress>50 && progress <=80){
			setmWaterColor(getResources().getColor(R.color.guard_orange));
		}else if (progress>80 && progress <=90){
			setmWaterColor(getResources().getColor(R.color.guard_yellow));
		}else if (progress>90 ){
			setmWaterColor(getResources().getColor(R.color.guard_green));
		}
		setWaterBgColor(getResources().getColor(R.color.guard_blue));
	 ...
	 * @param mProgressStr
	 */
	public void setProgressStr(String mProgressStr){
	 ...
	 */
	//TODO 需优化，不波动，显示基本数字
	public void isWait4Dev(String mProgressStr){
		this.mShowNumerical=false;
		this.mProgressStr=mProgressStr;
		this.mProgress=0;
		this.mTextColor=0xff787878;
	}
